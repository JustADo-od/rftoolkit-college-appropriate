#thats a lot of imports.... and at the time im doing this i have zero idea what some of them do, lol
import os
import subprocess
import threading
import time
...
#NOTE - will need to add a separate folder for all the protocol scripts
#another menu thingy
    def adsb_menu(self):
        while True:
            os.system('clear')
    ...
    def install_readsb(self):
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                print("Could not install readsb")
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                #im so tired of making error handling D:
        except Exception as e:
    ...
    def start_adsb_monitoring(self):
        ...
        try:
            ...
            cmd = [
                readsb_path,
                '--device-type', 'hackrf',
                '--gain', '20', #whoever didnt add shortages for keys is a menace to society >:(
                '--freq', '1090000000',
                '--net',
                '--net-json-port', '30005', # jesus christ, look at this key
                '--stats-every', '10',
                '--lat', '0',
                '--lon', '0',
                '--interactive' # no comment, also 7th iteration of the command build, i love 10 years old github repos
            ]
            ...
            #NOTE: im so confused, what am i doing rn????
    ...
    def _process_adsb_output(self):
        ...
        while self.monitoring and self.adsb_process:
            try:
                ...
                #skip separator lines typed out by the thingy
                if line.startswith('---'):
                    header_lines += 1
                    continue
    ...
    def _parse_aircraft_line(self, line):
        ...
        try:
            ...
            if len(parts) > flight_index:
                ...
                # flight callsign should be alphanumeric(key word SHOULD be)
            ...
            # Skip parsing errors (ads b is an ancient protocol)
            pass
        ...
        if current_time - self.last_cleanup > 30:
            ...
            for hex_code, aircraft in self.aircraft_data.items():
                ...
                try:
                    ...
                    #remove the aircraft if the timestamp is broken
                    expired_hexes.append(hex_code)
            ...
            while True:
                ...
                if not self.aircraft_data:
                    ...
                    for aircraft in sorted_aircraft[:30]:  # Show max 30 aircraft
                        ...
                        # shorten the callsign if too long
                        if len(callsign) > 10:
