...
import sys
import datetime
# Note before coding - i have no idea if this even works nowadays, and it will not be great if it doesnt:D
class GPSSpoof:
    def __init__(self):
...
# hey - another cool menu ^_^
    ...
    # validate if the user set evertyhing up or run setup option first
    ...
    def show_gps_data_instructions(self):
        ...
        year_short = today.strftime("%y")  # two-digit year(and this too) Edit: where did i need this?????
    ...
    def setup_gps_sdr_sim(self):
	    ...
	    # now gps_sdr_sim, well that was fun 3 hours of my life, figuring out that the name of the utility changed
    ...
    def generate_gps_signal(self):
        ...
        try:
            ...
            if not brdc_file.exists():
                ...
                input("Press Enter to continue...")
                return
            # setting coordinates | Edit: i used multi_gps_sdr_sim instead pf gps_sdr_sim(the right one), needed to re-write everything above D':
            ...
            # Format: gps-sdr-sim -e brdc.dat -l lat,lon,alt -d duration -b 8 , 10th iteration of command build, idk how, idk why
            cmd = [
                './gps-sdr-sim',
                '-e', 'brdc.dat',
                '-l', f"{lat},{lon},{alt}",
...
# error handling if something random happens somehow :/
    ...
    def realtime_spoofing(self):
        ...
        try:
            ...
            # Adding repeat option for user to choose
            if repeat == 'y':
                cmd.append('-R')
